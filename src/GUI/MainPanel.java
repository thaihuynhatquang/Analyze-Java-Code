...
//        System.out.println("hello");
        for(Box index: listBox.values()){
            ...
            // draw  association
            boolean is2WayAssociation=false;
            boolean isHasA=false;
            for(Variable var: index.getData().getVariables()){
                String varType = var.getType();
                for(String key: listBox.keySet()){
                    if(varType.contains(key)){
                        g.drawLine(index.getRec().x,index.getRec().y,listBox.get(key).getRec().x,listBox.get(key).getRec().y);
                        isHasA=true;
                        break;
                    }
